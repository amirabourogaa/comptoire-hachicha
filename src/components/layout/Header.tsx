...
function SocialIcon({ type, size = 14 }: { type: string; size?: number }) {
  switch (type) {
    ...
    case 'whatsapp':
      return <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor"><path d="M20.52 3.48A11.86 11.86 0 0012.08 0C5.53 0 .2 5.33.2 11.88c0 2.09.55 4.13 1.59 5.93L.1 24l6.34-1.66a11.88 11.88 0 005.64 1.43h.01c6.55 0 11.88-5.33 11.88-11.88 0-3.18-1.24-6.16-3.45-8.41zm-8.44 18.25h-.01a9.86 9.86 0 01-5.03-1.38l-.36-.21-3.76.99 1-3.67-.23-.38a9.86 9.86 0 01-1.51-5.2C2.18 6.44 6.61 2 12.08 2a9.87 9.87 0 017.02 2.91 9.88 9.88 0 012.9 7.03c0 5.47-4.45 9.9-9.92 9.9zm5.43-7.42c-.3-.15-1.77-.87-2.04-.97-.27-.1-.47-.15-.67.15-.2.3-.77.97-.95 1.17-.17.2-.35.22-.65.07-.3-.15-1.26-.46-2.4-1.47-.89-.79-1.49-1.77-1.66-2.07-.17-.3-.02-.46.13-.61.13-.13.3-.35.45-.52.15-.17.2-.3.3-.5.1-.2.05-.37-.02-.52-.07-.15-.67-1.62-.92-2.22-.24-.58-.49-.5-.67-.51h-.57c-.2 0-.52.07-.8.37-.27.3-1.04 1.02-1.04 2.49s1.07 2.89 1.22 3.09c.15.2 2.1 3.21 5.09 4.5.71.31 1.27.49 1.71.63.72.23 1.37.2 1.89.12.58-.09 1.77-.72 2.02-1.42.25-.7.25-1.3.17-1.42-.07-.12-.27-.2-.57-.35z"/></svg>;
    case 'tiktok':
...
function MegaContactLinks({ links }: { links: { type: string; url: string }[] }) {
  const facebookUrl = links.find(link => link.type === 'facebook')?.url || 'https://www.facebook.com/';
  const instagramUrl = links.find(link => link.type === 'instagram')?.url || 'https://www.instagram.com/';
  ...
  return (
    <div className="hidden md:flex shrink-0 flex-col items-center justify-center gap-1 border-l border-navbar-text/15 pl-4 lg:pl-5">
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center gap-2 text-sm font-bold text-[#25D366] hover:text-[#55e889] transition-colors whitespace-nowrap"
        aria-label="Contacter sur WhatsApp au +216 23 623 000"
      >
      ...
      >
        {isScrollable ? (
          ...
          <div className="bg-navbar-bg text-navbar-text">
            ...
            {/* Main Bar */}
            <div className="container mx-auto px-4 lg:px-8">
              <div className="flex items-center gap-3 lg:gap-5 h-16 lg:h-20 min-w-0">
                {/* Mobile Menu Button */}
                <button
                  className="lg:hidden p-2 -ml-2 text-navbar-text"
                  onClick={() => setIsMenuOpen(!isMenuOpen)}
                ...
                {/* Main links: kept on the same line, before the logo */}
                <nav className="hidden lg:flex items-center gap-5 shrink-0" aria-label="Navigation principale">
                  <Link
                    to="/"
                    className="whitespace-nowrap text-sm font-medium text-navbar-text hover:text-primary transition-colors"
                  >
                    Accueil
                  </Link>
                  <Link
                ...
                {/* Cart: left of the logo */}
                <Link
                  to="/cart"
                  className="relative shrink-0 p-2.5 rounded-full text-navbar-text hover:bg-navbar-text/10 transition-all group"
                  aria-label="Panier"
                >
                  <ShoppingBag size={22} className="transition-transform group-hover:scale-110" />
                  {itemCount > 0 && (
                ...
                <MegaContactLinks links={activeSocialLinks} />
              ...
              <div className="px-6 py-4 border-b border-navbar-text/10">
                ...
                {isMega && (
                  <div className="mt-4 flex flex-col gap-2">
                    <a
                      href={activeSocialLinks.find(link => link.type === 'whatsapp')?.url || 'https://wa.me/21623623000'}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-2 text-sm font-bold text-[#25D366]"
                    >
                      <SocialIcon type="whatsapp" size={19} />
                      <span className="text-navbar-text">+216 23 623 000</span>
                    </a>
                    <div className="flex items-center gap-4">
                      <a href={activeSocialLinks.find(link => link.type === 'facebook')?.url || 'https://www.facebook.com/'} target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="text-[#1877F2]"><SocialIcon type="facebook" size={18} /></a>
                      <a href={activeSocialLinks.find(link => link.type === 'instagram')?.url || 'https://www.instagram.com/'} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="text-[#E4405F]"><SocialIcon type="instagram" size={18} /></a>
                    </div>
                  </div>
                )}
              </div>
